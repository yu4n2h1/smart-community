...
<template>
	...
	<view class="W">
		<!-- 列表空提示 -->
		<view v-if="data.length === 0" class="item-null">暂无内容</view>
...
const load = () =>{
	...
	// 查询 问卷
	if(!getLocalData('feedback-list')) return
	data.push(...getLocalData('feedback-list'))
...
const showDetail = () =>{
	uni.navigateTo({
		url: '/pages/feedback/feedbackDetail'
	})
}
	...
	uni.showModal({
		...
		success: res =>{
			if(res.confirm){
				...
				data.push(...d)
				try{
					setLocalData('feedback-list', data)
					uni.showToast({
						title: '删除成功!',
						icon: 'success',
						duration: 2000,
