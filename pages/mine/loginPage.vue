<template>
  <view class="background-container">
	...
	<view style="margin-top: 60vh;">
		...
		<button @click="show = true" class="overlay-button" style="margin-top: 20px; background-color: aquamarine;">游客登陆</button>
	...
	<u-popup :show="showDeal1" @close="close" @open="open" mode="center" :round="10">
		<view class="pop-block" style="height: 450px;">
			<view style="height: 50px; text-align: center; line-height: 50px; font-size: 20px;">《用户服务协议》</view>
			<scroll-view style="height: 400px;" scroll-y="true">
				<view style="width: 90%; margin-left: 5%;">
					<!-- 这里不知道为什么不能通过上面那个view的方式对文字进行居中处理，所以先这样搞 -->
			...
			<button @click="showDeal1 = false" style="z-index: 2; color: orange; width: 100%;">知道了</button>
		</view>
	</u-popup>
	
	<u-popup :show="showDeal2" @close="close" @open="open" mode="center" :round="10">
		<view class="pop-block" style="height: 450px;">
			<view style="height: 50px; text-align: center; line-height: 50px; font-size: 20px;">《隐私政策》</view>
			...
			<button @click="showDeal2 = false" style="z-index: 2; color: orange; width: 100%;">知道了</button>
		</view>
	</u-popup>
	
	<u-popup :show="show" @close="close" @open="open" mode="center" :round="10">
        <view class="pop-block">
			<text style="display: flex; justify-content: center; margin-top: 10px;">邀请您补全个人信息（昵称、头像）</text>
            <view style="display: flex; flex-direction: row; align-items: center; margin-top: 15px;">
				<text style="margin-left: 10%;">头像：</text>
			...
			<view style="display: flex; flex-direction: row; align-items: center; margin-top: 15px;">
				<text style="margin-left: 10%;">昵称：</text>
				<input type="nickname" class="weui-input" placeholder="请输入用户名" v-model="nickname" style="padding-left: 12%;"/>
			</view>
			
			<view style="display: flex; flex-direction: row; align-items: center; margin-top: 15px;">
				<!-- <button  open-type="getPhoneNumber" @getphonenumber="getUserPhone" :disabled="explainName == '' ? false : true">手机号授权</button> -->
				<!-- <button  open-type="getPhoneNumber" @getphonenumber="getUserPhone" class="pop-block-bottom-button">手机号授权</button> -->
				<button class="pop-block-bottom-button" @click="loginConfirm">确认</button>
				<button @click="show = false" class="pop-block-bottom-button">拒绝</button>
			</view>
...
<script setup>
	...
	const title = ref("标题")
	const content = ref('A如果本《协议》中的任何条款无论因何种原因完全或部分无效，或不具有执行力，或违反任何适用的法律，则该条款被视为删除，但本《协议》的其余条款仍应有效，并且有约束力。B我公司有权随时根据有关法律法规的变化、公司经营状况和经营策略的调整等情况修改本《协议》。修改后的协议将随附于新版本软件。当发生有关争议时，以最新的协议文本为准。如果不同意改动的内容，用户可以自行删除本产品。如果用户继续使用本产品，则视为您接受本产品的变动。A如果本《协议》中的任何条款无论因何种原因完全或部分无效，或不具有执行力，或违反任何适用的法律，则该条款被视为删除，但本《协议》的其余条款仍应有效，并且有约束力。B我公司有权随时根据有关法律法规的变化、公司经营状况和经营策略的调整等情况修改本《协议》。修改后的协议将随附于新版本软件。当发生有关争议时，以最新的协议文本为准。如果不同意改动的内容，用户可以自行删除本产品。如果用户继续使用本产品，则视为您接受本产品的变动。')
	const avatarUrl = ref('');
	const nickname = ref('');
	const logCode = ref('');
	const phone = ref('');
	...
	const loginConfirm = async () => {
	  try {
		...
		const avatarByte = await getAvatarData(openid);
